...
4. **Generate the review report** using the `skill_export_static_review` tool to create a standalone HTML file:
   ```
   Call skill_export_static_review with:
     workspace: <workspace>/iteration-N
     skillName: "my-skill"
     outputPath: <workspace>/iteration-N/review.html
   ```
   For iteration 2+, also pass `previousWorkspace: <workspace>/iteration-<N-1>`.

   This creates a static HTML file that can be opened in any browser. The user can review the results and download feedback as `feedback.json`.

Note: please use the `skill_export_static_review` tool to create the static HTML report; there's no need to write custom HTML.
...
3. Generate the static HTML review report using `skill_export_static_review`, passing `previousWorkspace` pointing at the previous iteration
4. Tell user the HTML file location for them to review
...
- With the user, evaluate the outputs:
  - Create benchmark.json (via `skill_aggregate_benchmark`) and generate static HTML report (via `skill_export_static_review`) to help the user review them
  - Run quantitative evals
- Repeat until you and the user are satisfied
- Install the final skill for the user.

Please add steps to your TodoList (using the todowrite tool), if you have such a thing, to make sure it happens. Please specifically put "Generate static HTML review report (via skill_export_static_review) so human can review test cases" in your TodoList to make sure it happens.
